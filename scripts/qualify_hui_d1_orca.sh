...
evidence_dir="$(mktemp -d /tmp/hui-d1-orca.XXXXXX)"
cleanup() {
...
orca_pid=$!
sleep 2
node test/accessibility/hypermedia_ui_phase_d1_orca.mjs >"${evidence_dir}/browser.json"
sleep 2
rg --quiet "View refreshed" "${evidence_dir}/speech.log"
rg --quiet "Fleet" "${evidence_dir}/speech.log"
printf 'HUI-D1 named Orca speech and focus checks passed. Ephemeral evidence: %s\n' "${evidence_dir}"
