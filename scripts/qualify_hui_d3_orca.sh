...
evidence_dir="$(mktemp -d /tmp/hui-d3-orca.XXXXXX)"
cleanup() {
...
orca_pid=$!
sleep 2
node test/accessibility/hypermedia_ui_phase_d3_orca.mjs >"${evidence_dir}/browser.json"
sleep 2
...
printf 'HUI-D3 named Orca speech and focus checks passed. Ephemeral evidence: %s\n' "${evidence_dir}"
