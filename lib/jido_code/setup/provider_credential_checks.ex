defmodule JidoCode.Setup.ProviderCredentialChecks do
  ...
  def run(previous_state \\ nil) do
    ...
    checker
    |> safe_invoke_checker(%{checked_at: checked_at, previous_statuses: previous_statuses})
    |> normalize_report(checked_at, previous_statuses)
  end
  ...
  def from_state(state) when is_map(state) do
    ...
    credentials =
      ...
      |> normalize_credentials(checked_at, %{})
  ...
  @doc false
  def default_checker(%{checked_at: checked_at, previous_statuses: previous_statuses})
    ...
    Enum.map(provider_definitions(), fn definition ->
      ...
      {status, detail, remediation, verified_at} =
        case credential_value(definition) do
          nil ->
            {:not_set, definition.not_set_detail, definition.not_set_remediation, nil}

          credential ->
            if definition.valid?.(credential) do
              {:active, definition.active_detail, "Credential is active.", checked_at}
            else
              {:invalid, definition.invalid_detail, definition.invalid_remediation, nil}
            end
        end
  ...
  defp normalize_report(%{credentials: credentials} = report, default_checked_at, previous_statuses)
       when is_list(credentials) do
    checked_at =
      report
      |> map_get(:checked_at, "checked_at")
    ...
    normalized_credentials = normalize_credentials(credentials, checked_at, previous_statuses)
  ...
  defp normalize_report(credentials, default_checked_at, previous_statuses) when is_list(credentials) do
    normalize_report(%{credentials: credentials}, default_checked_at, previous_statuses)
  end

  defp normalize_report({:error, reason}, _default_checked_at, _previous_statuses) do
    checker_error_report(reason)
  end

  defp normalize_report(other, _default_checked_at, _previous_statuses) do
    checker_error_report({:invalid_checker_result, other})
  end
    ...
    credentials =
      Enum.map(provider_definitions(), fn definition ->
        %{
          ...
          name: definition.name,
          status: :invalid,
          previous_status: :not_set,
          transition: transition_label(:not_set, :invalid),
          detail: "Unable to verify provider credential: #{inspect(reason)}",
  ...
  defp normalize_credentials(credentials, checked_at, previous_statuses) do
    credentials
    |> Enum.with_index()
    |> Enum.map(fn {credential, index} ->
      normalize_credential(credential, checked_at, previous_statuses, index)
    end)
  end

  defp normalize_credential(credential, default_checked_at, previous_statuses, index)
    ...
    %{
      ...
      verified_at:
        ...
        |> normalize_verified_at(status, default_checked_at),
      checked_at:
        credential
        |> map_get(:checked_at, "checked_at")
        |> normalize_checked_at(default_checked_at)
  ...
  defp normalize_credential(_credential, default_checked_at, previous_statuses, index) do
    definition = default_provider_definition(index)
    previous_status = Map.get(previous_statuses, definition.provider, :not_set)
  ...
  defp normalize_verified_at(value, :active, default_checked_at) do
    value
    |> normalize_checked_at(default_checked_at)
  end

  defp normalize_verified_at(_value, _status, _default_checked_at), do: nil
